...
rectangle width height p = abs (fst p) <= width / 2 && abs (snd p) <= height / 2
...
plot width height region = intercalate "\n"
    [[if inside (fromIntegral x, fromIntegral (-y)) region then '*' else '.' | x <- [-width..width]] | y <- [-height..height]]
...
{-
    it seems that foldl, foldr and foldl' yield the same result,
    but I think foldl' would be more efficient here
-}
...
{-
    *** TODO ***
    ...
    Răspuns: in cazul apelarii functiei inside pe regiunea determinata de circles,
             corpul functiei circles nu mai este evaluat dupa ce a fost
             gasit cercul in care se afla punctul. Acest fapt este vizibil in cazul unui apel
             precum:
             > inside (6, 0) $ circles 10000000000
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: functia se va evalua ad infinitum
-}
...
bfs start expand = explore (S.singleton start) [(start, 0)]
    where
    explore _ [] = []
    explore visited queue =
        let (node, dist) = head queue
            neighbors = filter (`S.notMember` visited) (expand node)
            newVisited = foldl' (flip S.insert) visited neighbors
            newStates = map (, dist + 1) neighbors
            newQueue = tail queue ++ newStates
        in (node, dist) : explore newVisited newQueue
...
regionAvoidingBfs start region = bfs start (\(x, y) -> filter (not . region) [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)])
