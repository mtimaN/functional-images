...
instance Show RegionAST where
    ...
    show reg = foldRegionAST combiner reg 0
      where
        combiner :: RegionCombiner (Int -> String)
        combiner reg l = replicate (2 * l) ' ' ++ case reg of
          (FromPoints ps) -> "FromPoints " ++ show ps
          (Rectangle w h) -> "Rectangle " ++ show w ++ " " ++ show h
          (Circle r) -> "Circle " ++ show r
          (Complement r) -> "~\n" ++ r (l + 1)
          (Union r1 r2) -> "+\n" ++ r1 (l + 1) ++ "\n" ++ r2 (l + 1)
          (Intersection r1 r2) -> "*\n" ++ r1 (l + 1) ++ "\n" ++ r2 (l + 1)
          (Transform t r) -> show t ++ "\n" ++ r (l + 1)
...
instance Num RegionAST where
    fromInteger n = R $ FromPoints [(fromInteger n, fromInteger n)]
...
instance Functor TransformationShape where
    ...
    fmap f (Combine ts) = Combine (fmap f ts)
    fmap _ (Translation tx ty) = Translation tx ty
    fmap _ (Scaling f) = Scaling f
...
instance Functor RegionShape where
    ...
    fmap f region = case region of
      FromPoints ps -> FromPoints ps
      Rectangle x y -> Rectangle x y
      Circle r -> Circle r
      Complement r -> Complement (f r)
      Union r1 r2 -> Union (f r1) (f r2)
      Intersection r1 r2 -> Intersection (f r1) (f r2)
      Transform t r -> Transform t (f r)
...
{-
    *** TODO ***
    ...
    Răspuns: Cealalta componenta nu mai este calculata.(ramane un "promise")
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Transform t (a, b)) = (0 + a, basicTransformationCount t + b)
    combiner (Union (a, b) (c, d)) = (a + c, b + d)
    combiner (Intersection (a, b) (c, d)) = (a + c, b + d)
    combiner (Complement r) = r
    combiner _ = (1, 0)
